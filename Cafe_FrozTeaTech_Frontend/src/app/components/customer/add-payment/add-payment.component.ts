...
export class AddPaymentComponent implements OnInit {
  ...
  constructor(private activeRoute: ActivatedRoute,private router: Router,private FrozTeaTechservice: FrozTeaTechService) {}
  ngOnInit(): void {
  ...
  onSubmit() {
    this.FrozTeaTechservice.addPayment(this.payment, this.generatedOrderId, this.cust).subscribe(
      () => {
  ...
  getOrderByOrderId() {
    this.FrozTeaTechservice.getorderbyid(this.generatedOrderId).subscribe(
      (data) => { 
        console.log(data);
        this.order = data;
