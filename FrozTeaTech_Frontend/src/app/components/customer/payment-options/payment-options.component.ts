...
export class PaymentOptionsComponent implements OnInit {
  ...
  constructor(
    ...
    private FrozTeaTechservice: FrozTeaTechService
  ) { }
  ngOnInit(): void {
  ...
  getOrderDetails() {
    ...
    if (this.orderId) {
      this.FrozTeaTechservice.getorderbyid(this.orderId).subscribe(
        (data) => {
          this.order = data;
  ...
  deleteOrder() {
    if (this.orderId) {
      ...
      if (confirmation) {
        this.FrozTeaTechservice.deleteOrder(this.orderId).subscribe(
          () => {
